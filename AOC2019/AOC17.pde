...
void setup() {
  ...
  while (robot.hasOutput()) {
    prin(robot.getOutput());
  }
  in = getInputs();
  for (int i=0; i<in.length(); i++) {
    robot.input(in.charAt(i));
    while (robot.hasOutput()) {
      prin(last = robot.getOutput());
    }
  }
...
String getInputs() {
  String path = getPath();
  String[] comp = getComponents(path);
  String ret = path;
  
  ret = ret.replaceAll(comp[0], "A");
  ret = ret.replaceAll(comp[1], "B");
  ret = ret.replaceAll(comp[2], "C");
  for (int i=0; i<3; i++) {
    ret += "\n"+comp[i];
  }
  ret += "\nn\n";
...
String getPath() {
  ...
  pos = findPlayer();
  char p = map[(int)pos.x][(int)pos.y];
  switch(p) {
  case '<': 
    dir = new PVector(-1, 0); 
    break;
  ...
  int steps = 0;
  while (true) {
    if (map(pos.x+dir.x, pos.y+dir.y) == '#') {
      steps++;
      pos = pos.add(dir);
    } else {
      dir = rot(dir, PI/2.0);
      char r = map((pos.x+dir.x), (pos.y + dir.y));
      dir = rot(dir, PI*1.0);
      char l = map((pos.x + dir.x), (pos.y+dir.y));
      dir = rot(dir, PI/2.0);
      ...
      if (r=='#') {
        ...
        ret += steps;
        return setColons(ret.substring(1));
      }
      dir = new PVector((int)dir.x, (int)dir.y);
      steps = 0;
    }
  }
}

PVector rot(PVector dir, float r) {
  dir.rotate(r);
  PVector ret = new PVector((int)dir.x, (int)dir.y);
  return ret;
}

char map(float x, float y) {
  char ret;
  try {
    ret = map[(int)x][(int)y];
  } 
...
String[] getComponents(String in) {
  String[] ret = new String[3];
  String path = in.replaceAll(",", "");
  for (int i=1; i<min(MAXLEN,path.length()); i++) {
    path = in.replaceAll(",", "");
    String a = path.substring(0, i);
    for (int j=1; j<min(MAXLEN,path.length()); j++) {
      path = in.replaceAll(",", "").replaceAll(a, "");
      String b = path.substring(0, j);
      for (int k=1; k<min(MAXLEN,path.length()); k++) {
        path = in.replaceAll(",", "").replaceAll(a, "").replaceAll(b, "");
        String c = path.substring(0, k);
        if (in.replaceAll(setColons(a), "").replaceAll(setColons(b), "").replaceAll(setColons(c), "").replaceAll(",","").equals("") && !(b.endsWith("L") || b.endsWith("R"))) {
          ret[0] = setColons(a);
          ret[1] = setColons(b);
          ret[2] = setColons(c);
        }
