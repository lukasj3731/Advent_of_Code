...
void setup() {
  ...
  for (int i=0; i<l1.size(); i++) {
    if (l2.contains(l1.get(i))) {
      Point p1 = l2.get(l2.indexOf(l1.get(i)));
      Point p2 = l1.get(i);
      int m = min(d1, Dist1(p1));
      d1 = (m>0)? m : d1;
      m = min(d2, Dist2(p1, p2));
      d2 = (m>0)? m : d1;
    }
  }
  println("Task 1: "+d1 + "\nTask 2: "+ d2);
...
void doOp(String op, ArrayList<Point> list) {
  String i = op;
  char d = i.charAt(0);
  i = i.substring(1);
  ...
  for (int a=0; a<l; a++) {
    list.add(new Point((int)(x+dir.x*a), (int)(y+dir.y*a), ind));
    ind++;
  }
  x += dir.x*l;
  y += dir.y*l;
...
private class Point {
  ...
  int y;
  int val;
  public Point(int xin, int yin, int valin) {
    x = xin;
    y = yin;
    val = valin;
  }
